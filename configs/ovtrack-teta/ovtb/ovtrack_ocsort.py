model = dict(
    ...
    roi_head=dict(
        type='OVTrackRoIHead',
        finetune_track=True,
        prompt_path= 'saved_models/pretrained_models/ovtb_class_prompt.pt',
...
data_root = 'data/ovmot/'

img_scale = (800, 1333)
img_norm_cfg = dict(
    mean=[123.132, 119.537, 114.178], std=[55.112, 53.927, 53.798], to_rgb=True)
train_pipeline = [
...
data = dict(
    ...
    val=dict(
        type=dataset_type,
        classes='data/tao/annotations/lvis_classes_v1.txt',
    ...
    test=dict(
        type=dataset_type,
        classes='data/ovtb/ovtb_classes.txt',
        ann_file='data/ovtb/ovtb_ann.json',
        img_prefix='data/ovtb/OVT-B/',
        ref_img_sampler=None,
        pipeline=test_pipeline)
