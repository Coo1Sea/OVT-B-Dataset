...
def bisoftmax(embeds, memo_embeds, temperature=-1, match_with_cosine=True):
    sims = cal_similarity(
        embeds,
        memo_embeds,
        method="dot_product",
    ...
    if match_with_cosine:
        scores = (scores + cos_scores) / 2
    return scores

def cosine(embeds, cls_embeds, memo_embeds, memo_cls_embeds, temperature=-1, contrastive_thr=0.5):
    cos_scores = cal_similarity(embeds, memo_embeds, method="cosine")
    ...
    cat_same = cls_sims > contrastive_thr
